...
LWR_Subcanopy_CLM_Abisko_HM = vertcat(LW_in_bc_CLM(EP1(1):EP1(end),:),LW_in_bc_CLM(EP2(1):EP2(end),:),...
    LW_in_bc_CLM(EP3(1):EP3(end),:),LW_in_bc_CLM(EP4(1):EP4(end),:),LW_in_bc_CLM(EP5(1):EP5(end),:));

save('MLRdata_Abisko_HM.mat','LAI_Abisko','PAI_Abisko','Hourly_Time_Abisko',...
    'Cosine_Solar_Angle_Abisko','SWR_Incoming_Abisko','LWR_Atmosphere_Abisko',...
    'Sky_Emissivity_Abisko','LWR_Subcanopy_Observations_Abisko','LWR_Subcanopy_CLM_Abisko_HM')