...
Latitude = 68+21/60+20/60/60;
Elevation = 388;
% met tower?
Height_of_Hum_Obs = 2;
Height_of_Temp_Obs = 2;
Height_of_Wind_Obs = 2;
...
%}
PFT = 9;    % birch -> broadleaf deciduous boreal trees (or temperate?)
Vegetation_Height = 3.5;

%{
from Nick's script:
PAI from hemi photos using van Gardinen et al 1999 - see Reid et al. 2014
data were cut and pasted from Tim's spreadsheets LwDays 080714.xlsx (LW 
positions) and SVPAIgraphs.xlsx (SW positions)
%}
% PAI at each of the four LW positions at each site
VAI_C = [0.367229 0.14447 0.702209 0.540339];

Fraction_LAI_Of_VAI = 0;
LAI = Fraction_LAI_Of_VAI * VAI_C;
SAI = (1 - Fraction_LAI_Of_VAI) * VAI_C;


%-----------------------  only for SNOWPACK-2LHM  ------------------------%
Basal_Area_of_Forest_Stand = 0.0006077; % 0.00078 according to Reid et al. (2014b) though
Tree_Diameter = 0.0376;
...
Snow_Fraction = 1;
Sand_Fraction = 0.539902;
Clay_Fraction = 0.190610;
Fraction_Organic_Matter = 0.0655;
...
Air_Temperature_Above_Vegetation = T_air_RH_1h;
...
Precipitation = Precip_adjust/3600;

% ground
Soil_Water = GWL;
%{
Since the actual snow depth is not important for
the Toy Model and the difference should be rather small anyway, snow depth
from the open is used without being modified.
%}
Snow_Depth_Below_Vegetation = z_snow;
Surface_Temperature = nan(length(T_surf_C),11);
for j=1:11
    bla = T_surf_C-mean(T_surf_C);
    bums = mean(T_surf_C)-273.15;
    Surface_Temperature(:,j) = min(273.15,bla+273.15+(0.8+(j-1)*0.04)*bums);
end
Fraction_Frozen_Soil = 1;   % soil temperatures consistently below 0°C
...
T_veg_CLM = nan(length(time_1h),4);        % CLM4.5 vegetation temperature [?]
T_veg_CLM(1,:) = Air_Temperature_Above_Vegetation(1);
T_leaf_SP = nan(length(time_1h),4);        % SP-2LHM leaf layer temperature [?]
T_leaf_SP(1,:) = Air_Temperature_Above_Vegetation(1);
T_trunk_SP = nan(length(time_1h),4);       % SP-2LHM trunk layer temperature [?]
T_trunk_SP(1,:) = Surface_Temperature(1,6);

% output variables
Cosine_Solar_Angle = nan(size(time_1h));
SW_net_veg_CLM = nan(length(time_1h),4);
LW_in_bc_CLM = nan(length(time_1h),4,11);
...
LW_in_bc_SP = nan(length(time_1h),4,11);
...
% call Driver
        [Cosine_Solar_Angle(t),CanInt(t,i),CanWetFrac(t,i),SW_net_veg_CLM(t,i),LW_in_bc_CLM(t,i,j),...
            LW_out_ac_CLM(t,i),T_veg_CLM(t,i),T_air_wc_CLM(t,i),T_ref2m_CLM(t,i),...
            EB_CLM(t,i,:,:),EB_SP_leaf(t,i,:,:),EB_SP_trunk(t,i,:,:),...
            SW_net_can_SP(t,i),SW_net_trunk_SP(t,i),LW_in_bc_SP(t,i,j),LW_out_ac_SP(t,i),...
        ...
        SNOWPACKwithinCLM45driver(Latitude,Longitude,Elevation,JulDay(t),...
            ...
            Snow_Depth_Below_Vegetation(t),Snow_Fraction,Fraction_Frozen_Soil,Surface_Albedo(t),...
            Surface_Temperature(t,j),T_veg_CLM(t-1,i),T_leaf_SP(t-1,i),T_trunk_SP(t-1,i),...
...
save('LWsub_Abisko_Tsurf.mat','LW_in_bc_C_1h','LW_in_bc_CLM','LW_in_bc_SP')
toc