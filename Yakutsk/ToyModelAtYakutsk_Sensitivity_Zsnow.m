...
Snow_Depth_Below_Vegetation = nan(length(T_topsoil_98_1h_filled),11);
for j=1:11
    Snow_Depth_Below_Vegetation(1:3195,j) = max(0,(0.8+(j-1)*0.04)*0.2);
end
...
Snow_Age = zeros(length(Snow_Depth_Below_Vegetation),1);
...
%}
    ...
    if Snow_Depth_Below_Vegetation(t,j) > 0
        Snow_Fraction = 1;
    end
    
    if Snow_Depth_Below_Vegetation(t,j) > 0 && Snowfall(t) == 0
...
% call Driver
    ...
    SNOWPACKwithinCLM45driver(Latitude,Longitude,Elevation,JulDay(t),...
        ...
        Snow_Depth_Below_Vegetation(t,j),Snow_Fraction,Fraction_Frozen_Soil(t),alb_surf_98(t),...
...
save('LWsub_Yakutsk_Zsnow.mat','SW_in_ac_98_1h_filled','SW_out_bc_98_1h_filled',...
    'SW_in_bc_98_1h_filled','Rnet_bc_98_1h_filled','T_surf_98_1h_filled',...
    'LW_in_bc_CLM_98','LW_in_bc_SP_98')
toc