...
SAI = 1.71;
%{
% guided by CLM4.5 monthly LAI
LAIweights = [0 0 0 0 0.179487179487180 0.666666666666667 1 0.794871794871795 0.384615384615385 0 0 0];
LAImonthly = LAIweights*LAImax;
LAI_98 = nan(size(time_98_1h));
for l=1:length(LAI_98)
    LAI_98(l) = InterpolateMonthlyPAI(1998,str2num(datestr(time_98_1h(l),'mm')),...
        str2num(datestr(time_98_1h(l),'dd')),LAImonthly);
end
LAI_00 = nan(size(time_00_1h));
for l=1:length(LAI_00)
    LAI_00(l) = InterpolateMonthlyPAI(2000,str2num(datestr(time_00_1h(l),'mm')),...
        str2num(datestr(time_00_1h(l),'dd')),LAImonthly);
end
%}
LAI = 0;
...
Diffuse_Fraction = nan(length(frac_diff_98),11);
for j=1:11
    Diffuse_Fraction(:,j) = min(1,max(0,(0.8+(j-1)*0.04)*frac_diff_98));
end
...
%{
Snow_Depth_Below_Vegetation = zeros(size(T_topsoil_98_1h_filled));
for t=1:length(time_98_1h)
    if T_topsoil_98_1h_filled(t) <= 273.15
        Snow_Depth_Below_Vegetation(t) = 1;
    end
end
%}
...
T_veg_CLM_98(1049) = Air_Temperature_Above_Vegetation(1049) + 0.0098*(Height_of_Temp_Obs-Vegetation_Height);
T_leaf_SP_98 = nan(size(time_98_1h));        % SP-2LHM leaf layer temperature [K]
T_leaf_SP_98(1049) = Air_Temperature_Above_Vegetation(1049) + 0.0098*(Height_of_Temp_Obs-Vegetation_Height);
...
LW_in_bc_CLM_98 = nan(length(time_98_1h),11);
...
LW_in_bc_SP_98 = nan(length(time_98_1h),11);
...
% direct-diffuse partitioning
    SW_In_Diffuse(1) = SW_In_Above_Vegetation(t)*Diffuse_Fraction(t,j);
    SW_In_Diffuse(2) = 0;
    SW_In_Direct = SW_In_Above_Vegetation(t)-SW_In_Diffuse(1);
    SW_In_Direct(2) = 0;
...
% call Driver
    [Cosine_Solar_Angle_98(t),CanInt_98(t),CanWetFrac_98(t),SW_net_veg_CLM_98(t),LW_in_bc_CLM_98(t,j),...
        LW_out_ac_CLM_98(t),T_veg_CLM_98(t),T_air_wc_CLM_98(t),T_ref2m_CLM_98(t),...
        EB_CLM_98(t,:,:),EB_SP_leaf_98(t,:,:),EB_SP_trunk_98(t,:,:),...
        SW_net_can_SP_98(t),SW_net_trunk_SP_98(t),LW_in_bc_SP_98(t,j),LW_out_ac_SP_98(t),...
...
save('LWsub_Yakutsk_Fdiff.mat','SW_out_bc_98_1h_filled','SW_in_bc_98_1h_filled',...
    'Rnet_bc_98_1h_filled','T_surf_98_1h_filled','LW_in_bc_CLM_98','LW_in_bc_SP_98')
toc