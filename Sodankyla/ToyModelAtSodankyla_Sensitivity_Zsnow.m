...
LAI = Fraction_LAI_Of_VAI * VAI;
SAI = (1 - Fraction_LAI_Of_VAI) * VAI;
...
Snow_Depth_Below_Vegetation = nan(length(z_snow_open),11);
for j=1:11
    Snow_Depth_Below_Vegetation(:,j) = max(0,(0.8+(j-1)*0.04)*0.75*z_snow_open);
end
...
Snow_Age = zeros(length(Snow_Depth_Below_Vegetation(:,1)),1);
...
% call Driver
        ...
        SNOWPACKwithinCLM45driver(Latitude,Longitude,Elevation,JulDay(t),...
            ...
            Snow_Depth_Below_Vegetation(t,j),Snow_Fraction,Fraction_Frozen_Soil(t),Surface_Albedo(t),...
...
save('LWsub_Sodankyla_Zsnow.mat','LW_in_bc_C_1h','LW_in_bc_CLM','LW_in_bc_SP')
toc