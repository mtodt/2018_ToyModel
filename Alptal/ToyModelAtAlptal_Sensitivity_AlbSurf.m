...
%}
    ...
    forc_sol_dif = SW_In_Above_Vegetation(t,1)*Diffuse_Fraction(t,1);
...
% surface albedo parameterization via snow ageing
    ...
    Surface_Albedo(t,1) = min(1,max(0,(0.8+(j-1)*0.04)*Surface_Albedo(t,1)));
...
%}
        ...
        forc_sol_dir(2) = 0;    % no nir radiation
        forc_sol_dif = SW_In_Above_Vegetation(t,i)*Diffuse_Fraction(t,i);
        forc_sol_dif(2) = 0;    % no nir radiation
...
% surface albedo parameterization via snow ageing
    ...
    Surface_Albedo(t,i) = min(1,max(0,(0.8+(j-1)*0.04)*Surface_Albedo(t,i)));
...
save('LWsub_Alptal_AlbSurf.mat','LW_in_bc_1h_all','LW_in_bc_CLM','LW_in_bc_SP')
toc