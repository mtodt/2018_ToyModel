...
PAI = 4.1;  % average "LAI" alongthe rail
...
Diffuse_Fraction = nan(length(frac_diff_1h_all(:,1)),length(frac_diff_1h_all(1,:)),11);
for i=1:11
    Diffuse_Fraction(:,:,i) = min(1,max(0,(0.8+(i-1)*0.04)*frac_diff_1h_all));
end
...
LW_in_bc_CLM = nan(length(time_1h_all(:,1)),length(time_1h_all(1,:)),11);
...
LW_in_bc_SP = nan(length(time_1h_all(:,1)),length(time_1h_all(1,:)),11);
...
marginal.
%}
    forc_sol_dir(1) = SW_In_Above_Vegetation(t,1)*(1-Diffuse_Fraction(t,1,j));
    forc_sol_dir(2) = 0;    % no nir radiation
    forc_sol_dif(1) = SW_In_Above_Vegetation(t,1)*Diffuse_Fraction(t,1,j);
...
[Cosine_Solar_Angle(t,1),CanInt(t,1),CanWetFrac(t,1),SW_net_veg_CLM(t,1),LW_in_bc_CLM(t,1,j),...
    LW_out_ac_CLM(t,1),T_veg_CLM(t,1),T_air_wc_CLM(t,1),T_ref2m_CLM(t,1),...
    EB_CLM(t,1,:,:),EB_SP_leaf(t,1,:,:),EB_SP_trunk(t,1,:,:),...
    SW_net_can_SP(t,1),SW_net_trunk_SP(t,1),LW_in_bc_SP(t,1,j),...
...
marginal.
%}
        forc_sol_dir(1) = SW_In_Above_Vegetation(t,i)*(1-Diffuse_Fraction(t,i,j));
        forc_sol_dir(2) = 0;    % no nir radiation
        forc_sol_dif(1) = SW_In_Above_Vegetation(t,i)*Diffuse_Fraction(t,i,j);
        forc_sol_dif(2) = 0;    % no nir radiation
...
% surface albedo parameterization via snow ageing
    ...
    [Cosine_Solar_Angle(t,i),CanInt(t,i),CanWetFrac(t,i),SW_net_veg_CLM(t,i),LW_in_bc_CLM(t,i,j),...
...
save('LWsub_Alptal_Fdiff.mat','LW_in_bc_1h_all','LW_in_bc_CLM','LW_in_bc_SP')
toc