...
% ground
Soil_Water = nan(length(SWProxy_1h_all(:,1)),length(SWProxy_1h_all(1,:)),11);
for j=1:11
    Soil_Water(:,:,j) = min(1,max(0,(0.8+(j-1)*0.04)*SWProxy_1h_all));
end
...
SNOWPACKwithinCLM45driver(Latitude,Longitude,Elevation,JulDay(t,1),...
    ...
    Fraction_Organic_Matter,Soil_Water(t,1,j),SoilWaterFlag,CanWetFrac(t-1,1),CanInt(t-1,1));
end
...
% surface albedo parameterization via snow ageing
    ...
    SNOWPACKwithinCLM45driver(Latitude,Longitude,Elevation,JulDay(t,i),...
        ...
        Fraction_Organic_Matter,Soil_Water(t,i,j),SoilWaterFlag,CanWetFrac(t-1,i),CanInt(t-1,i));
    end
end
%}
end

save('LWsub_Alptal_SWC.mat','LW_in_bc_1h_all','LW_in_bc_CLM','LW_in_bc_SP')
toc