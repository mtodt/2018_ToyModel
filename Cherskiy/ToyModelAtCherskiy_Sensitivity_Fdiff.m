...
Diffuse_Fraction = nan(length(frac_diff),11);
for j=1:11
    Diffuse_Fraction(:,j) = min(1,max(0,(0.8+(j-1)*0.04)*frac_diff));
end
...
% direct-diffuse partitioning
    ...
    SW_In_Direct(1) = SW_In_Above_Vegetation(t)*(1-Diffuse_Fraction(t,j));
...
save('LWsub_Cherskiy_Fdiff.mat','LW_in_bc_1h','LW_in_bc_CLM','LW_in_bc_SP')
