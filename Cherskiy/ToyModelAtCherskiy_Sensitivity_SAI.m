...
SAI_calc = 3.7*LSA;                      % number of trees per m^2, see below
SAI = nan(11,1);
for j=1:11
    SAI(j) = max(0,(0.8+(j-1)*0.04)*SAI_calc);
end
...
% call Driver
    ...
    SNOWPACKwithinCLM45driver(Latitude,Longitude,Elevation,JulDay(t),...
        Timestep_CLM,Timestep_SP,PFT,LAI,SAI(j),Vegetation_Height,...
...
save('LWsub_Cherskiy_SAI.mat','LW_in_bc_1h','LW_in_bc_CLM','LW_in_bc_SP')
