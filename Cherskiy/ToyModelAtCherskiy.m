...
%}
%{
LAI_obs = 0.793699; % late July/early August
LAI_CLM = [0 0    0 0 0 0.670 2.940 2.470    0 0    0 0];
SAI_CLM = [0.600 0.600    0.600 0.600    0.600 0.600    0.600 0.840 2.870 1.470 0.700 0.600];
LAIweights = LAI_CLM/max(LAI_CLM);
LAImonthly = LAIweights*LAI_obs;
LAI = nan(size(time_1h));
for l=1:length(LAI)
    LAI(l) = InterpolateMonthlyPAI(2017,str2num(datestr(time_1h(l),'mm')),...
        str2num(datestr(time_1h(l),'dd')),LAImonthly);
end
%{
Since SAI doesn't change until August and we're only simulating for snowmelt
season, we assume a constant SAI.
%}
Fraction_LAI_Of_VAI = LAI_CLM./(LAI_CLM + SAI_CLM);
Fraction_LAI_Of_VAI = (Fraction_LAI_Of_VAI(7) + Fraction_LAI_Of_VAI(8))/2;
VAI = LAI_obs/Fraction_LAI_Of_VAI;
SAI = VAI * (1-Fraction_LAI_Of_VAI);
%}
...
% call Driver
    [Cosine_Solar_Angle(t),CanInt(t),CanWetFrac(t),SW_net_veg_CLM(t),LW_in_bc_CLM(t),...
    ...
    SNOWPACKwithinCLM45driver(Latitude,Longitude,Elevation,JulDay(t),...
        ...
        Fraction_Organic_Matter,Soil_Water,SoilWaterFlag,CanWetFrac(t-1),CanInt(t-1));    
end
