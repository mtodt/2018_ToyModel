...
the critical value.
%}
Snow_Depth_Below_Vegetation = nan(length(Snow_Fraction),11);
for s=1:length(Snow_Depth_Below_Vegetation)
    if Snow_Fraction(s) > 0
        for j=1:11
            Snow_Depth_Below_Vegetation(s,j) = max(0,(0.8+(j-1)*0.04)*0.2);
        end
    end
end
...
save('LWsub_Cherskiy_Zsnow.mat','LW_in_bc_1h','LW_in_bc_CLM','LW_in_bc_SP')
