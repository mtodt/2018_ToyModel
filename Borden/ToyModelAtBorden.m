...
LW_in_bc_CLM = nan(length(JulDay_1h),2);
LW_in_bc_CLM_comb = nan(size(JulDay_1h));
...
LW_in_bc_SP = nan(length(JulDay_1h),2);
...
% surface albedo parameterization via snow ageing
    ...
    for i=1:2
        [Cosine_Solar_Angle(t),CanInt(t,i),CanWetFrac(t,i),...
            SW_net_veg_CLM(t,i),LW_in_bc_CLM(t,i),LW_out_ac_CLM(t,i),...
    ...
    LW_in_bc_CLM_comb(t) = LW_in_bc_CLM(t,1) * PFT_Fractions(1) ...
        + LW_in_bc_CLM(t,2) * PFT_Fractions(2);
end
