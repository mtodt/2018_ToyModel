...
PFT_Fractions = nan(2,11);
for j=1:11
    PFT_Fractions(1,j) = min(1,max(0,(0.8+(j-1)*0.04)*0.1867));
    PFT_Fractions(2,j) = 1-PFT_Fractions(1,j);
end
...
Private communication with Paul Bartlett:
- max PAI about 5.6
%}
% LAI_max = 4.6;
% LAI = nan(size(JulDay_1h)); % or VAI? - calculated true LAI after woody ratio (17%)
% folowing by eye from Figure 2 in Croft et al. (2015)
% LAI(3120) = 4.5;
% LAI(3000) = 2.25;
% LAI_BDTT = [0 0 0 0 0.85 3.7 3.9 3.65 3.55 2.3 0 0];
LAI_NETT = [2.85 2.8 2.85 2.85 3.3 3.8 3.95 3.75 3.6 2.8 2.9 2.9];
% SAI_BDTT = [0.8 0.75 0.75 0.75 0.75 0.75 0.8 0.85 0.85 1.6 3.15 1.55];
SAI_NETT = [0.85 0.85 0.85 0.85 0.85 0.85 0.85 0.85 0.85 1.2 0.85 0.85];
f_LAI_NETT = mean(LAI_NETT(6:9))/mean(LAI_NETT(6:9)+SAI_NETT(6:9));

PAI_tot_post = 5.6;     % post-leafout total PAI
PAI_tot_pre = 1.36;     % post-leafout total PAI
LAI_tot_post = 4.6;     % post-leafout total LAI  

PAI = nan(2,11); LAI = nan(2,11); SAI = nan(2,11);
for j=1:11
    LAI(1,j) = (LAI_tot_post - (PAI_tot_post-PAI_tot_pre))/PFT_Fractions(1,j);
    PAI(1,j) = LAI(1,j)/f_LAI_NETT;
    SAI(1,j) = PAI(1,j)-LAI(1,j);
    SAI(2,j) = (PAI_tot_pre - PAI(1,j)*PFT_Fractions(1,j))/PFT_Fractions(2,j);
    LAI(2,j) = 0.05; % 0 doesn't work for latent heat flux
end
% SAI = [0.48 1.1];
...
%---------------------------  Initializations  ---------------------------%
%-------------------------------------------------------------------------%
Snow_Age = zeros(size(Snow_Fraction));
...
T_veg_CLM(10,:) = Air_Temperature_Above_Vegetation(10) + 0.0098*(Height_of_Temp_Obs-Vegetation_Height);
T_leaf_SP = nan(length(JulDay_1h),2);        % SP-2LHM leaf layer temperature [K]
T_leaf_SP(10,:) = Air_Temperature_Above_Vegetation(10) + 0.0098*(Height_of_Temp_Obs-Vegetation_Height);
...
% surface albedo parameterization via snow ageing
    ...
    for i=1:2
        ...
        SNOWPACKwithinCLM45driver(Latitude,Longitude,Elevation,JulDay_1h(t),...
            Timestep_CLM,Timestep_SP,PFT(i),LAI(i,j),SAI(i,j),Vegetation_Height,...
...
save('LWsub_Borden_fPFT.mat','LW_in_bc_1h','LW_in_bc_CLM_comb','LW_in_bc_SP')
