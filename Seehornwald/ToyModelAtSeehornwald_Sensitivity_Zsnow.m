...
Snow_Depth_Below_Vegetation = nan(length(z_snow_forest_1h),11);
for j=1:11
    Snow_Depth_Below_Vegetation(:,j) = max(0,(0.8+(j-1)*0.04)*z_snow_forest_1h);
end
...
% snow age for surface albedo parameterization
        if Snow_Depth_Below_Vegetation(t,j) > 0 && forc_snow == 0
            Snow_Age(t) = Snow_Age(t-1) + 1/24;   % snow age in days for ageing
        elseif Snow_Depth_Below_Vegetation(t,j) < Snow_Depth_Below_Vegetation(t-1,j)
            Snow_Age(t) = Snow_Age(t-1) + 1/24;   % snow age in days for ageing
        end
...
% surface albedo parameterization via snow ageing
        ...
        Snow_Depth_Below_Vegetation(t,j),Snow_Fraction(t),Fraction_Frozen_Soil,...
...
save('LWsub_Seehornwald_Fsnow.mat','LW_in_bc_1h','LW_in_bc_CLM','LW_in_bc_SP')
toc